  ...
  useEffect(() => {
    const fetchPortfolio = async () => {
      ...
      for (const stock of holdings) {
        const quote = await fetchStockDetails(stock.symbol);
...
const UserDashboard = () => {
  ...
  useEffect(() => {
    const fetchPortfolio = async () => {
      ...
      try {
        ...
        for (const stock of userStocks) {
          const symbol = stock.symbol.includes(".") ? stock.symbol : `${stock.symbol}.NS`;
          ...
          if (mergedHoldings[stock.symbol]) {
            mergedHoldings[stock.symbol].quantity += stock.quantity;
          } else {
            mergedHoldings[stock.symbol] = {
              symbol: stock.symbol,
          ...
          mergedHoldings[stock.symbol].current = current.toFixed(2);
          mergedHoldings[stock.symbol].marketValue = marketValue.toFixed(2);
          mergedHoldings[stock.symbol].change = (current - (stock.avgPrice || 0)).toFixed(2);
    ...
    const syncHandler = () => fetchPortfolio();
  ...
  return (
    <Box m="20px">
      <Box display="flex" justifyContent="space-between" alignItems="center">
        <Header
          title="📈 My Live Portfolio"
          subtitle={`Welcome back, ${user?.name || "Investor"}!`} 
        />
        <Tooltip title="Export your holdings summary as CSV">
          <Button
            onClick={downloadHoldingsAsCSV}
            sx={{
              backgroundColor: colors.greenAccent[600],
      ...
      <Box mt="20px" p="25px" borderRadius="12px" backgroundColor={colors.primary[400]} boxShadow="0px 2px 10px rgba(0,0,0,0.1)">
        <Typography variant="h4" fontWeight="600" color={colors.greenAccent[400]}>
          ₹{portfolioValue.toLocaleString("en-IN")}
        </Typography>
        <Typography variant="body1" color={colors.grey[100]} mt="8px">
      ...
      <Box mt="30px" display="flex" flexDirection={{ xs: "column", md: "row" }} gap="20px">
        <Box flex={1} height="350px" backgroundColor={colors.primary[400]} borderRadius="12px" p="20px" boxShadow="0px 2px 10px rgba(0,0,0,0.08)">
          <Typography variant="h6" color={colors.grey[100]} mb="10px">
            🧩 Portfolio Allocation
          </Typography>
          <ResponsivePie
        ...
        <Box flex={1} backgroundColor={colors.primary[400]} borderRadius="12px" p="20px" maxHeight="350px" overflow="auto" boxShadow="0px 2px 10px rgba(0,0,0,0.08)">
          <Typography variant="h6" color={colors.grey[100]} mb="16px">
            💼 Your Holdings
          </Typography>
          <Divider sx={{ borderColor: colors.primary[500], mb: 2 }} />
  ...
  useEffect(() => {
    const fetchPortfolio = async () => {
      ...
      try {
        ...
        for (const stock of userStocks) {
          const rawSymbol = stock.symbol.includes(".") ? stock.symbol : `${stock.symbol}.NS`;
          ...
          mergedHoldings[key].change = (current - (stock.avgPrice || 0)).toFixed(2);
